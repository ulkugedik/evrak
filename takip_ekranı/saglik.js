 ...
 * 3. Geliştirici: Sağlık ve Aşılama Takip Gelen Formları Ekranı
 */

// Global Veri Tabanı Yardımcısı
const getDB = () => {
...
let currentTabFilter = "all"; // all, eval-pending, vaccine-list
...
window.initSaglikPortali = function(container) {
    ...
    container.innerHTML = `
        <div class="form-tracking-layout">
            ...
            <main class="details-panel glass-card animate-fade-in" id="health-details-pane">
...
function renderHealthInboxList() {
    ...
    if (currentTabFilter === 'eval-pending') {
        // Tetkik yapılmış ama henüz hekim değerlendirmesi bekleyenler
    ...
    filtered.forEach(s => {
        ...
        let statusBadgeText = s.health.evaluationStatus;
        let statusClass = 'status-new';
        if (s.health.evaluationStatus === 'Bağışık') {
            statusClass = 'status-ok';
        } else if (s.health.evaluationStatus === 'Aşı Gerekli') {
        ...
        li.innerHTML = `
            <div class="inbox-item-header">
                ...
                <span class="status-dot-badge ${statusClass}">${statusBadgeText}</span>
            </div>
            <div class="inbox-item-body">
                <span>No: ${s.id} | Aşı Dozu: ${s.health.doses.length}</span>
...
// Seçilen öğrencinin sağlık form detaylarını sağ panele yükler
function loadHealthFormDetails(studentId) {
    ...
    pane.innerHTML = `
        <div class="details-wrapper animate-fade-in">
            <header class="details-header">
                <div>
                    <h2>${student.name} - Sağlık Onay Formu</h2>
                    <span class="student-no-badge">Öğrenci No: ${student.id} | Bölüm: ${student.department}</span>
                </div>
            ...
            <div class="details-grid">
                <!-- Kart 1: Hepatit Tetkik Değerlendirme Formu (Table 4 Üst Kısım) -->
                    ...
                    <div id="hepatit-form-fields" class="${student.health.hepatitisTestDone ? '' : 'hidden'}">
                        ...
                        <div class="input-field margin-bottom-12">
                            ...
                            <div class="file-action-row">
                                <span class="file-name-text">${student.health.testFile ? `📄 ${student.health.testFile}` : '❌ Rapor Yüklenmemiş'}</span>
                                ${student.health.testFile ? `<button class="btn-text-preview" onclick="alert('Rapor dosyası açılıyor...'); return false;">👁 Önizle</button>` : ''}
                            </div>
                        </div>
                ...
                <!-- Kart 2: Aşı Takvimi ve Doz Girişi Formu (Table 4 Alt Kısım) -->
    ...
    // Hepatit tetkik yapıldı mı check değişimi
    ...
    // Aşı Gerekli seçildiğinde listeyi işaretle
    ...
    // Aşı kartı tablosunu render et
    renderDosesList(student.health.doses);

    // Aşı bölümü görünürlük ayarı
...
// Hepatit Değerlendirmesini Kaydet
function saveHepatitEvaluation() {
    ...
    showToast(`${student.name} için Hepatit sağlık bilgileri güncellendi.`);
    renderHealthInboxList();
}
...
function addNewDose() {
    ...
    showToast(`${student.name} için ${doseNo}. Doz aşı formu eklendi.`);
    renderHealthInboxList();
}
...
function injectStyles() {
    ...
    style.innerHTML = `
        /* Arama ve Liste Sütun Yerleşimi */
        ...
        .form-inline-status-dose { background: #1e293b; color: #fff; border: 1px solid rgba(255,255,255,0.1); border-radius: 4px; padding: 2px 4px; font-size: 0.7rem; outline: none; cursor: pointer; }
        ...
        .btn-save-details { background: linear-gradient(135deg, #10b981, #059669); color: #fff; border: none; padding: 10px 20px; border-radius: 6px; font-size: 0.85rem; font-weight: 600; cursor: pointer; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.2); transition: all 0.2s; }
        .btn-save-details:hover { transform: translateY(-1px); box-shadow: 0 6px 16px rgba(16, 185, 129, 0.3); }
        .btn-save-details.btn-secondary-form { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #fff; box-shadow: none; width: 100%; }
